...
from third_party.pcme.models.pie_model import MultiHeadSelfAttention
import sys

class UncertaintyModuleImage(nn.Module):
    def __init__(self, num_embeds, d_in, d_out, d_h):
        super().__init__()
        self.num_embeds = num_embeds
        self.attention = MultiHeadSelfAttention(self.num_embeds, d_in, d_h)
    ...
    def forward(self, out, x, pad_mask=None):
        ...
        if self.num_embeds > 1:
            fc_out = fc_out.unsqueeze(1).repeat(1, self.num_embeds, 1)
...
class UncertaintyModuleText(nn.Module):
    ...
    def forward(self, x, pad_mask=None, lengths=None):
        ...
        if self.num_embeds > 1:
            gru_out = gru_out.unsqueeze(1).repeat(1, self.num_embeds, 1)
