...
def batchwise_cdist(samples1, samples2, eps=1e-6, metric = 'euclidian'):
    ...
    if metric == 'cosine':
        samples1 = l2_normalize(samples1)
        samples2 = l2_normalize(samples2)
        return (1 - samples1 @ samples2.transpose(-1, -2)).view(batch_size, -1)
...
class MCSoftContrastiveLoss(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.config = config
        shift = config.criterion.init_shift * torch.ones(1)
        negative_scale = config.criterion.init_negative_scale * torch.ones(1)
        ...
        self.uniform_lambda = config.criterion.uniform_lambda
        self.vib_beta = config.criterion.vib_beta
        self.use_cosine_distance = True if config.criterion.distance_metric == 'cosine' else False
    ...
    def pairwise_sampling(self, anchors, candidates):
        ...
        if self.use_cosine_distance:
            cdist = batchwise_cdist(anchors, selected, metric='cosine')
        else:
            cdist = batchwise_cdist(anchors, selected, metric='euclidian')
    ...
    def match_prob(self, image_features, caption_features):
        # sampled_image_features = sample_gaussian_tensors(image_features, image_logsigma, 7)
        # sampled_caption_features = sample_gaussian_tensors(caption_features, caption_logsigma, 7)

        sampled_image_features, sampled_caption_features = image_features, caption_features
        if self.use_cosine_distance:
            distance = batchwise_cdist(sampled_image_features, sampled_caption_features, metric='cosine')
        else:
            distance = batchwise_cdist(sampled_image_features, sampled_caption_features, metric='euclidian')
    ...
    def forward(self, image_features, caption_features, image_logsigma, caption_logsigma, **kwargs):
        ...
        loss = 2*t2i_loss['loss'] + self.uniform_lambda * uniform_loss + self.vib_beta * vib_loss

        loss_dict = {
                    # 'i2t_loss': i2t_loss['loss'].item(),
                     ...
                     'match_loss': 2*t2i_loss['loss'].item(),
